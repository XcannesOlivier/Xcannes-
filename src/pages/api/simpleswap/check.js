...
export default async function handler(req, res) {
  ...
  const requestCheck = async ({ tickerFrom: tf, networkFrom: nf, tickerTo: tt, networkTo: nt }) => {
    return simpleSwapRequest("/exchanges/check", {
      query: {
        fixed: String(fixed),
  ...
  try {
    ...
    try {
      const data = await requestCheck(original);
      ...
      for (const nf of fromCandidates) {
        for (const nt of toCandidates) {
          ...
          try {
            const data = await requestCheck(candidate);
