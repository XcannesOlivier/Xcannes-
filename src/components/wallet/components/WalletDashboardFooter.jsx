...
export default function WalletDashboardFooter({ onScan } = {}) {
  ...
  return (
    <div className="mt-auto shrink-0 z-20 bg-elevated [--bg-elevated:#090c0d] md:[--bg-elevated:unset] border-t-2 border-white/30 md:border-t md:border-white/10">
      ...
      <div className="md:hidden relative h-14 pb-[env(safe-area-inset-bottom)]">
        ...
        {onScan ? (
          <button
            type="button"
            onClick={onScan}
            className="absolute left-1/2 top-2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 rounded-full bg-elevated ring-2 ring-white/35 shadow-[0_-10px_22px_rgba(0,0,0,0.35)] flex items-center justify-center text-white/70 hover:text-white/90 transition-colors"
