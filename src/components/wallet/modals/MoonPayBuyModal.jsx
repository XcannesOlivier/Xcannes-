      ...
      {step === 'form' && (
        <div className={wizardStep === 2 ? 'space-y-4 md:space-y-5' : 'space-y-5'}>
          ...
          {/* Wallet + Title (merged) */}
              ...
              <p className="block text-[16px] md:text-base font-orbitron font-bold text-white mb-3">
                {wizardStep === 1 ? (
                  ...
                  <>
                    <span className="text-[20px] tracking-[0.14em]">
                      {t('moonpay_buy_asset_details_prefix', 'Détails de la transaction')}
                    </span>
                  </>
                )}
              </p>
              {String(walletLabel || '').trim() ? (
                <div className="flex items-center gap-2 mb-1">
                  <span
                    ...
                    aria-hidden
                  />
                  <p className="min-w-0 text-[16px] md:text-[17px] text-white font-semibold truncate">{walletLabel}</p>
                </div>
              ) : null}
          ...
          <button
            ...
            disabled={continueDisabled}
            className={[
              "hidden md:flex items-center justify-center w-full h-12 rounded-xl text-white text-xl font-semibold transition-all duration-150",
              continueDisabled
                ? "opacity-45 cursor-not-allowed"
                : "hover:scale-[1.01] active:scale-[0.98]",
            ].join(" ")}
