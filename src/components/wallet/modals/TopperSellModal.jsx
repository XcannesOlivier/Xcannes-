...
import { TOPPER_ACTIVE_STORAGE_KEY, setTopperActive } from "./topperShared";

const TopperSellModal = ({
  isOpen,
  onClose,
  walletAddress,
  walletLabel = "",
  ...
  const [cryptoCurrency, setCryptoCurrency] = useState("XRP");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/topper/generate-sell-url", {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(
          data?.error?.message ||
            data?.message ||
            t("topper_error_generate_sell_url", {
              defaultValue: "Failed to generate Topper sell URL.",
            }),
        );
      }
      ...
      console.error("Error generating Topper sell URL:", err);
      setError(
        err?.message ||
      ...
      {embedded ? null : (
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold text-white">
            {t("topper_sell_title", { defaultValue: "Vendre (Topper)" })}
          </h3>
      ...
      {step === "iframe" && iframeUrl ? (
        ...
        >
          ...
          <iframe
            ...
            allowFullScreen
            title={t("topper_widget_title_sell", { defaultValue: "Topper Widget" })}
          />
        </div>
      ) : step === "loading" ? (
        ...
        <div className="flex-1 min-h-0">
          <div className="text-sm text-white/70 mb-4">
            {t("topper_sell_subtitle", {
              defaultValue: "Vendez des cryptos et recevez du fiat avec Topper (Uphold).",
            })}
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setCryptoCurrency(String(e.target.value || "").toUpperCase())
                }
            ...
            <div>
              <label className="block text-xs text-white/60 mb-1">
                {t("topper_amount_crypto", { defaultValue: "Montant (crypto)" })}
              </label>
              <input
                value={amount}
            ...
            >
              {loading
                ? t("loading", "Loading...")
                : demoMode
                  ? t("topper_action_simulate_sell", { defaultValue: "Simuler la vente" })
                  : t("topper_action_continue_sell", { defaultValue: "Continuer" })}
            </button>
