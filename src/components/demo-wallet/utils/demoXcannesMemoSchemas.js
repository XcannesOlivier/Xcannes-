...
const SCHEMA_TO_TYPE = Object.entries(XCANNES_MEMO_SCHEMAS).reduce(
  (acc, [type, meta]) => {
...
function normalizeMemoTypeMarker(value) {
  ...
  if (lower === "wallet_label" || lower.includes("wallet-label"))
    return "wallet_label";
  if (lower === "currency_line" || lower.includes("currency-line"))
  ...
  if (
    ...
    return "conversion";
  if (lower === "allocation_adjust" || lower.includes("allocation"))
    return "allocation_adjust";
...
function parseOptionalNumber(value, { min = null, minExclusive = false } = {}) {
  ...
  if (min != null) {
    if (minExclusive && parsed <= min)
      return { ok: false, value: parsed, provided: true };
    if (!minExclusive && parsed < min)
      return { ok: false, value: parsed, provided: true };
  }
...
function getXcannesMemoSchema(type) {
  return XCANNES_MEMO_SCHEMAS[type]
    ? { type, ...XCANNES_MEMO_SCHEMAS[type] }
    : null;
}
...
function normalizeCurrencyLinePayload(payload, errors) {
  ...
  const allocatedAfterRes = parseOptionalNumber(payload?.allocatedRlusdAfter, {
    min: 0,
  });
  ...
  if (allocatedAfterRes.value != null)
    normalized.allocatedRlusdAfter = allocatedAfterRes.value;
  return normalized;
}
...
function normalizeConversionPayload(payload, errors) {
  ...
  const lineStatesRaw = Array.isArray(payload?.lineStates)
    ? payload.lineStates
    : Array.isArray(payload?.line_states)
      ? payload.line_states
      : null;
  let lineStates = null;
  if (lineStatesRaw && lineStatesRaw.length > 0) {
    ...
    lineStatesRaw.forEach((entry) => {
      const currencyCode = normalizeCurrencyCode(
        entry?.currencyCode ?? entry?.currency,
      );
      if (!currencyCode) {
        errors.push("conversion.lineStates.currencyCode");
        return;
      }
      const amountRes = parseRequiredNumber(
        entry?.allocatedRlusdAfter ?? entry?.allocated_after,
        {
          min: 0,
        },
      );
  ...
  if (amountRlusdGrossRes.value != null)
    normalized.amountRlusdGross = amountRlusdGrossRes.value;
  if (amountBaseRes.value != null) normalized.amountBase = amountBaseRes.value;
  if (amountQuoteRes.value != null)
    normalized.amountQuote = amountQuoteRes.value;
  if (fxRateRes.value != null) normalized.fxRate = fxRateRes.value;
  if (fxSource) normalized.fxSource = fxSource;
  if (spreadRlusdRes.value != null)
    normalized.spreadRlusd = spreadRlusdRes.value;
...
function normalizePayreqPayload(
  payload,
  errors,
  { requireOrigin = false } = {},
) {
  ...
  if (displayAmountRes.value != null)
    normalized.displayAmount = displayAmountRes.value;
...
function normalizeAllocationAdjustPayload(payload, errors) {
  ...
  const defaultAction = defaultActionRaw
    ? defaultActionRaw.toLowerCase()
    : null;
  ...
  if (adjustments && adjustments.length > 0) {
    ...
    adjustments.forEach((entry) => {
      ...
      const entryAction = entryActionRaw
        ? entryActionRaw.toLowerCase()
        : defaultAction;
      ...
      const amountRes = parseRequiredNumber(
        entry?.amountRlusd ?? entry?.amount,
        {
          min: 0,
          minExclusive: true,
        },
      ...
      const allocatedAfterRes = parseOptionalNumber(
        entry?.allocatedRlusdAfter,
        { min: 0 },
      );
      ...
      if (entryAction && entryAction !== defaultAction)
        normalizedEntry.action = entryAction;
      if (allocatedAfterRes.value != null)
        normalizedEntry.allocatedRlusdAfter = allocatedAfterRes.value;
  ...
  const amountRes = parseRequiredNumber(
    payload?.amountRlusd ?? payload?.amount,
    {
      min: 0,
      minExclusive: true,
    },
  ...
  if (!allocatedAfterRes.ok)
    errors.push("allocation_adjust.allocatedRlusdAfter");
...
function normalizeMoonpayPayload(payload, errors) {
  ...
  const amountRes = parseOptionalNumber(
    payload?.amount ?? payload?.displayAmount,
    { min: 0, minExclusive: true },
  );
  if (!amountRes.ok) errors.push("moonpay.amount");

  const amountRlusdRes = parseOptionalNumber(payload?.amountRlusd, {
    min: 0,
    minExclusive: true,
  });
...
function validateXcannesMemoPayload(payload, options = {}) {
  ...
  const allowLegacy =
    options.allowLegacy != null ? options.allowLegacy : mode !== "create";
  const errors = [];
...
export {
  ...
  getXcannesMemoSchema,
  inferXcannesMemoType,
  validateXcannesMemoPayload,
  buildXcannesMemoPayload,
  createXcannesMemoPayload,
