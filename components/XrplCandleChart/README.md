...
```jsx
// ex: app/layout.jsx ou pages/_app.jsx
...
// ajuster le chemin selon votre arborescence
...
- `hooks/useMarketData.js` : fetch historique (Mongo klines), abonnement WebSocket (`XcannesWSContext`) pour XRPL orderbooks et Pyth, polling ticker 24h (30s), calcul bougie courante, mode FX EOD via REST (`getFxEod`).
...
- Vérif structure/export : `npm run check:chart` exécute `checkChart.js`.

Importez depuis le barrel `components/XrplCandleChart` pour éviter les chemins profonds :  
`import XrplCandleChart, { ChartHeader, useMarketData, IndicatorsToolbar } from "../components/XrplCandleChart";`
