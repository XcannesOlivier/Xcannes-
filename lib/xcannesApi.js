...
const rawApiUrl = (process.env.NEXT_PUBLIC_XCANNES_API_URL || "").trim();

function resolveApiBaseUrl() {
  const trimmed = (rawApiUrl || "").replace(/\/$/, "");

  if (!isProdStage) {
    return trimmed || "http://localhost:3003";
  }

  if (!trimmed) {
    throw new Error(
      "NEXT_PUBLIC_XCANNES_API_URL doit être défini en production."
    );
  }

  // Accepter HTTP en production (temporaire - à sécuriser avec HTTPS plus tard)
  if (!trimmed.startsWith("https://") && !trimmed.startsWith("http://")) {
    throw new Error(
      `URL backend invalide: ${trimmed}. Utilisez un endpoint HTTP/HTTPS valide.`
    );
  }
...
class XcannesAPI {
  ...
  async request(endpoint, options = {}) {
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
      try {
        ...
        lastError = error;
        console.warn(`[XcannesAPI] Tentative ${attempt + 1}/${retries} échouée:`, error.message);
  ...
  /**
   * Récupérer toutes les paires incluant display
   * @returns {Promise<Object>}
   */
  async getAllMarkets() {
    const result = await this.request(API_ENDPOINTS.MARKETS_ALL);
    return result.success ? result.data : { trading: [], display: [] };
  }
  ...
  /**
   * Récupérer le prix d'une paire forex (Pyth)
   * @param {string} pair - Format: EUR_USD
   * @returns {Promise<Object>}
   */
  async getForexPrice(pair) {
    const result = await this.request(`/api/v1/forex?symbol=${pair}`, {
      useCache: true,
      cacheTTL: 1000, // Cache 1s pour prix live (au lieu de 3s)
    });
    return result;
  }

  /**
   * Récupérer tous les prix forex
   * @returns {Promise<Object>}
   */
  async getAllForexPrices() {
    const result = await this.request('/api/v1/forex', {
      useCache: true,
      cacheTTL: 1000, // Cache 1s
    });
    return result;
  }

  /**
   * Récupérer le prix d'une commodity (Pyth)
   * @param {string} pair - Format: XAU_USD
   * @returns {Promise<Object>}
   */
  async getCommodityPrice(pair) {
    const result = await this.request(`/api/v1/commodities?symbol=${pair}`, {
      useCache: true,
      cacheTTL: 1000, // Cache 1s
    });
    return result;
  ...
  /**
   * Récupérer tous les prix commodities
   * @returns {Promise<Object>}
   */
  async getAllCommodityPrices() {
    const result = await this.request('/api/v1/commodities', {
      useCache: true,
      cacheTTL: 1000, // Cache 1s
    });
    return result;
  ...
  /**
   * Récupérer le prix d'une crypto (Pyth)
   * @param {string} pair - Format: BTC_USD
   * @returns {Promise<Object>}
   */
  async getCryptoPrice(pair) {
    const result = await this.request(`/api/v1/crypto?symbol=${pair}`, {
      useCache: true,
      cacheTTL: 1000, // Cache 1s
    });
    return result;
  ...
  /**
   * Récupérer tous les prix crypto
   * @returns {Promise<Object>}
   */
  async getAllCryptoPrices() {
    const result = await this.request('/api/v1/crypto', {
      useCache: true,
      cacheTTL: 1000, // Cache 1s
    });
    return result;
