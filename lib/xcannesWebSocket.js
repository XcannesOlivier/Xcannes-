...
function getDefaultWsUrl() {
  ...
  const port = (process.env.NEXT_PUBLIC_WS_PORT || process.env.WS_PORT || "3002").trim();
...
function resolveWsUrl() {
  ...
  // Accepter WS/ WSS en prod comme en dev
  if (!trimmed.startsWith("wss://") && !trimmed.startsWith("ws://")) {
    throw new Error(`URL WebSocket invalide: ${trimmed}. Utilisez un endpoint WS/WSS valide.`);
  }
...
class XcannesWebSocket {
  ...
  handleMessage(message) {
    ...
    console.log('[XcannesWS] 📨 Message reçu:', { type, channel, symbol: data?.symbol, hasListeners: this.listeners.has(type) });
    ...
    if (this.listeners.has(type)) {
      ...
      console.log(`[XcannesWS] 🎯 Dispatching ${type} à ${callbacks.size} listener(s)`);
      ...
      console.warn(`[XcannesWS] ⚠️ Aucun listener pour type: ${type}`);
    }
  }
